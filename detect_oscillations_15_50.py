...
def ripple(rate, fs):
    ...
    if meanr > np.power(10,-3):
	    ...
	    rAC, maxAC, tMaxAC, maxACR, tMaxACR, f, Pxx, avgRippleF, rippleP = (np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan)	
...
def gamma(f, Pxx):
    ...
    logic1 = isinstance(f, float)
    #print logic1
    logic2 = isinstance(Pxx, float)
    #print logic2
 
    if not(logic1) and not(logic2) :
	    f = np.asarray(f)
	    gammaS = np.where(15 < f)[0][0]
	    gammaE = np.where(f < 50)[0][-1]
	    f.tolist()
	    ...
	    gammaP = (tmp / power) * 100
            ...
            avgGammaF, gammaP = (np.nan, np.nan)
