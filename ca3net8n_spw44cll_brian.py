...
simtime = 1000.0 # Simulation time in ms (ideally 10s)
...
N_Pyr_all = 7200 # number of cells
N_Bas =400
N_Sli = 400
...
a_Pyr = -0.8*nS  # Subthreshold adaptation conductance
b_Pyr = 0.04*nA  # Spike-triggered adaptation
...
for post in range(NP):
	...
	if pre == post:
	    P2P_sub = Connection(Pyr_net[pre], Pyr_net[post], 'g_ampa',weight=J_PyrPot, sparseness=epsilon_Pyr,delay=delay_PyrExc)
	else:
	    P2P_sub = Connection(Pyr_net[pre], Pyr_net[post], 'g_ampa', weight=J_PyrExc, sparseness=epsilon_Pyr,delay=delay_PyrExc)
	print "Pyr --> Pyr\t\t"
...
#pyr_satemon = StateMonitor(Pyr_net[2], 'vm', record=[0,1])
...
fig2.savefig('avg_rate%.2fmf_%.2fslowpyr_%.2fpyrslow_%ddpot_%.2fjbasinh.png' % (p_rate_mf,J_PyrSlowInh,J_SliExc,dpot,J_BasInh))

fig3 = plt.figure(figsize=(10, 8))

ax = fig3.add_subplot(3,1,1)
ax.plot(np.linspace(900, 1000, len(Pyr_avg_rate[900:1000])), Pyr_avg_rate[900:1000], 'g-', linewidth=1.5)
ax.set_title('Pyr. pop avg rate | Mf exitation = %.2f Hz' % p_rate_mf)

ax = fig3.add_subplot(3,1,2)
ax.plot(np.linspace(900, 1000, len(popri.rate[900:1000])), popri.rate[900:1000], 'g-', linewidth=1.5)
ax.set_title('Bas. population rate')

ax = fig3.add_subplot(3,1,3)
ax.plot(np.linspace(900, 1000, len(poprs.rate[900:1000])), poprs.rate[900:1000], 'g-', linewidth=1.5)
ax.set_title('Slow inh. population rate')
ax.set_xlabel('Time [ms]')

fig3.savefig('avg_rate_last100_%.2fmf_%.2fslowpyr_%.2fpyrslow_%ddpot_%.2fjbasinh.png' % (p_rate_mf,J_PyrSlowInh,J_SliExc,dpot,J_BasInh))
